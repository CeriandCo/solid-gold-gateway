...
export function AurumWeeklyBriefSection({
  ...
  briefs: AurumEditorial[] | null;
}) {
  if (briefs !== null && briefs.length === 0) return null;

  const [featured, ...older] = briefs ?? [];
  ...
  return (
    <section id="weekly-brief" className="aurum-section aurum-weekly-brief" aria-labelledby="aurum-weekly-brief-title">
      <div className="aurum-container">
        ...
        {!featured ? (
          <p className="aurum-note-dek">Briefs are temporarily unavailable.</p>
        ) : (
          <>
            <BriefFeature
              featured={featured}
              isOpen={openSlug === featured.slug}
              onToggle={onToggle}
            />
      ...
      <div className="aurum-brief-feature__card">
        ...
        <div className="aurum-brief-feature__copy">
          ...
          <button
            ...
            onClick={() => onToggle(isOpen ? null : featured.slug)}
          >
            {isOpen ? "Close the brief ↑" : "Read the brief →"}
          </button>
        </div>
      </div>

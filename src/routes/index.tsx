    ...
    <main id="top" className="overflow-hidden bg-background">
      ...
      <section id="how-it-works" className="bg-ivory py-16 sm:py-20">
        <div className="mx-auto grid max-w-[1320px] items-center gap-10 px-5 sm:px-7 lg:grid-cols-[280px_1fr] lg:px-14">
          <div><p className="eyebrow text-gold">How it works</p><h2 className="section-title mt-4 text-forest">Five steps.<br />All online.<br />All simple.</h2><p className="body-copy mt-4 max-w-[220px] text-charcoal">From sign up to gold allocation in minutes.</p></div>
          <div>
            <div className="relative rounded-[18px] border border-beige bg-background px-8 py-10 sm:min-h-[310px] sm:px-20 sm:py-14">
              ...
              <div className="grid items-center gap-10 sm:grid-cols-[1fr_250px]">
                 <div><p className="ui-label text-forest">{String(step + 1).padStart(2, "0")} <span className="text-muted-foreground">/ 05</span></p><h3 className="mt-6 text-[1.875rem] font-normal leading-[1.08] text-forest sm:text-[2rem]">{currentStep[0]}</h3><p className="body-copy mt-4 max-w-[245px] text-charcoal">{currentStep[1]}</p></div>
      ...
      <section className="bg-ivory px-5 pb-16 sm:px-7 sm:pb-20">
        <div className="mx-auto grid max-w-[1240px] overflow-hidden rounded-[26px] bg-forest px-7 py-10 text-background sm:px-12 lg:grid-cols-[1fr_1.25fr_1fr] lg:items-center lg:py-12">
          <div><h2 className="section-title">A whole bar<br />asks you to buy<br />all of it.</h2><List bad items={["High upfront cost", "Less flexibility", "Storage and insurance to arrange", "Harder to sell small amounts"]} /></div>
          ...
          <div className="lg:pl-8"><h2 className="section-title">With SQOOT Pure,<br /><em className="font-normal text-gold">allocate</em> only what<br />you want.</h2><List items={["Start from as little as $25", "Buy or sell any amount", "Stored, insured and managed for you"]} /></div>
        </div>
      </section>
      ...
      <section id="fees" className="bg-background py-16 sm:py-20">
        <div className="mx-auto grid max-w-[1240px] gap-14 px-5 sm:px-7 lg:grid-cols-2 lg:px-10">
          <div className="lg:border-r lg:border-beige lg:pr-10"><p className="eyebrow text-gold">Fees</p><h2 className="section-title mt-3 text-forest">Simple, transparent fees</h2><p className="body-copy mt-3 text-muted-foreground">One simple fee. No hidden costs, no surprises.</p><div className="mt-7 grid rounded-lg border border-beige bg-ivory sm:grid-cols-3">{[[CircleDollarSign,"Allocation Fee","$3 per $100 allocated (3.00%)"],[ShieldCheck,"Annual Storage Fee","0.35% of value"],[FileCheck2,"Insured Storage","0.45% of value"]].map(([Icon,title,copy], i) => { const FeeIcon = Icon as typeof CircleDollarSign; return <div key={title as string} className={cn("px-5 py-7 text-center", i > 0 && "border-t border-beige sm:border-l sm:border-t-0")}><FeeIcon className="mx-auto text-gold" strokeWidth={1.4} /><p className="ui-label mt-4 text-forest">{title as string}</p><p className="mt-3 text-[0.6875rem] leading-[1.55] text-muted-foreground">{copy as string}</p></div>})}</div><p className="mt-4 text-[0.625rem] leading-[1.5] text-muted-foreground">*Fees are subject to change. See full Fee Schedule in FAQ.</p></div>
          <div id="faq"><p className="eyebrow text-gold">FAQ</p><h2 className="section-title mt-3 text-forest">Frequently asked questions</h2><div className="mt-5">{faqs.map(([question, answer], index) => { const isOpen = openFaq === index; return <div key={question} className="border-b border-beige"><button type="button" onClick={() => setOpenFaq(isOpen ? null : index)} className="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-3.5 text-left text-xs font-medium leading-[1.5] focus-visible:outline-2 focus-visible:outline-gold" aria-expanded={isOpen}><span>{question}</span><PlusIcon open={isOpen} /></button>{isOpen && <p className="body-copy pb-4 pr-8 text-muted-foreground">{answer}</p>}</div>})}</div></div>
        </div>
      </section>
      ...
      <section id="early-access" className="bg-forest py-10 text-background">
        <div className="mx-auto grid max-w-[1240px] items-center gap-8 px-5 sm:px-7 lg:grid-cols-[1fr_1.35fr] lg:px-10">
          <div className="flex items-center gap-6"><Mandala className="h-20 w-20 shrink-0 text-gold" /><div><h2 className="text-[2rem] font-normal leading-[1.08] text-gold sm:text-[2.125rem]">Be first in line.</h2><p className="body-copy mt-2">SQOOT Pure is launching soon.<br />Join the waitlist to get early access.</p></div></div>
          <form onSubmit={submitWaitlist} noValidate>
            <div className="grid gap-3 sm:grid-cols-[1fr_auto]"><label className="sr-only" htmlFor="waitlist-email">Email address</label><input id="waitlist-email" type="email" maxLength={254} value={email} onChange={(e) => { setEmail(e.target.value); setFormState("idle"); }} placeholder="Enter your email address" className="min-h-14 rounded-sm border border-transparent bg-background px-5 text-xs text-forest outline-none transition-shadow placeholder:text-muted-foreground focus:border-gold focus:ring-2 focus:ring-gold" aria-invalid={formState === "error"} aria-describedby="email-status" /><PrimaryButton type="submit" className="min-h-14 bg-gold px-9 text-background hover:bg-gold-dark">Get Early Access</PrimaryButton></div>
            <p id="email-status" className={cn("mt-3 flex items-center gap-2 text-[0.625rem] leading-[1.5]", formState === "error" && "text-error", formState === "success" && "text-gold")}><LockKeyhole size={14} className="text-gold" />{formState === "error" ? "Please enter a valid email address." : formState === "success" ? "You’re on the list. We’ll be in touch." : "We respect your privacy. Your information is safe with us."}</p>
          </form>
        </div>
      </section>

      <footer id="about" className="bg-background py-8"><div className="mx-auto grid max-w-[1240px] items-center gap-8 px-5 sm:px-7 lg:grid-cols-[auto_1fr_auto] lg:px-10"><Brand compact /><nav className="flex flex-wrap justify-center gap-x-8 gap-y-3" aria-label="Footer navigation">{nav.map(([label,href]) => <a key={label} href={href} className="text-[0.6875rem] font-medium hover:text-gold">{label}</a>)}</nav><div className="text-[0.625rem] leading-[1.6] text-muted-foreground lg:text-right"><p>© 2026 Fortress Gold Inc. All rights reserved.</p><p className="mt-2"><a href="#privacy" className="hover:text-gold">Privacy Policy</a><span className="mx-4">Terms of Service</span></p></div></div></footer>
    </main>
  );
}

function List({ items, bad = false }: { items: string[]; bad?: boolean }) {
  return <ul className="mt-6 space-y-3">{items.map((item) => <li key={item} className="flex items-start gap-3 text-[0.6875rem] leading-[1.55]"><span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full border border-gold text-gold">{bad ? <X size={11} /> : <Check size={11} />}</span>{item}</li>)}</ul>;
}
